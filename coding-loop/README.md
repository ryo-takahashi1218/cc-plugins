...
- プラグインを入れると、セッションの開始時に進め方の指示が自動で読み込まれる（約 800 文字）。この挙動が不要なら `/plugin` でプラグインを無効にする。
...
- スキル `coding-loop`（コードを書く・直す依頼で自動で発火する）
- エージェント `coding-loop:worker`（ファイルを書く作業役。指示された範囲だけ変更し、赤→緑の証拠を報告する）
- 進め方の指示（毎セッションの冒頭に自動で載る。手作業は作業役に委譲する、質問は選択式にする、プロジェクト固有の情報はプロジェクト内に置く、の 3 点）
